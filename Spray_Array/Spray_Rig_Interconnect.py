...
#        self.Center = False
    ...
    def hght(self, height):
        self.height = height
        return self
    def inter_span(self, interconnect_span):
        self.interconnect_span = interconnect_span
        return self
...
class CirclePartitions(BuildCirclePartitions):
    ...
    def circle_segment(self, is_second=False):
        ...
        if is_second:
            ...
            circle_segment = rotate_extrude(angle=self.second_angle, _fn=0)(rectangle)
        else:
            rectangle = square([self.radius_major, self.height])
            circle_segment = rotate_extrude(angle=self.angle, _fn=0)(rectangle)
    ...
    def circle_arc_segment(self, is_second=False):
        if is_second:
            ...
            inner_circle = rotate_extrude(angle=2*self.second_angle, _fn=0)(inner_rectangle)
            ...
            inner_circle = rotate_extrude(angle=2*self.angle, _fn=0)(inner_rectangle)
            circle_arc_segment = circle_segment - inner_circle
    ...
    def add_interconnect(self):
        # angular span of the interconnect as a fraction of the segment angle
        ...
        male_top = male_top.up(self.height/2 - self.wall_thickness/2 + epsilon)
...
class BuildSprayRig():
    ...
    def inlet_thick(self, inlet_thickness):
        ...
        return self
    def fastener_diam(self, fastener_diameter):
        self.fastener_diameter = fastener_diameter
        return self
    def fastener_gap(self, fastener_gap):
        self.fastener_gap = fastener_gap
        return self
    def fastener_thick(self, fastener_thickness):
        self.fastener_thickness = fastener_thickness
        return self
class SprayRig(BuildSprayRig, CirclePartitions):
    ...
    def nozzle_array(self):
        ...
        while i < total_track:
            ...
            while cur_angle < self.angle:
                theta = radians(270 + cur_angle)
                x = -track * sin(theta)
                y = track * cos(theta)
                hole = circle(r=self.nozzle_radius, _fn=6).translate([x, y])
                pattern = hole if pattern is None else pattern + hole
        ...
        if pattern is not None:
            nozzle_holes = linear_extrude(height=self.nozzle_height, center=True)(pattern)
            nozzle_holes = nozzle_holes.up(self.nozzle_height/2 + self.height/2)
            self.object = self.object - nozzle_holes
        return self
    '''
    add pagoda cone fasteners to the SprayRig roof
    (opposite side from the nozzle array) to fasten
    underneath the bucket lid. The cone barb compresses
    through a hole in the lid and springs back to retain.
    '''
    def add_lid_fasteners(self):
        cone_base_r = self.fastener_diameter / 2
        peg_rad = self.fastener_gap / 2
        peg = cylinder(r=peg_rad, h=self.fastener_gap, center=True, _fn=0)
        cone = cylinder(r1=cone_base_r, r2=0, h=self.fastener_thickness, center=True, _fn=0)
        cone = cone.up(self.fastener_gap/2 + self.fastener_thickness/2)
        fastener = peg + cone
        mid_radius = (self.radius_minor + self.radius_major) / 2
        fastener = fastener.right(mid_radius).up(1.5*self.height + self.fastener_gap/2)
        self.object += fastener.rotate([0, 0, self.angle / 6])
        self.object += fastener.rotate([0, 0, 5 * self.angle / 6])
        return self
    '''
...
#                                    h=self.tube_diameter, _fn=0, center=True)
...
#        self.object -= endcap_neg
    ...
    def add_pagoda(self):
        mean = (self.radius_major + self.radius_minor)/2
        interconnect = cylinder(r=self.tube_diameter/2, h=self.wall_thickness+self.tube_diameter/2, center=True, _fn=0)
        interconnect_neg = cylinder(r=self.tube_diameter/2-self.inlet_thickness, h=2*self.wall_thickness+self.tube_diameter/2, center=True, _fn=0)
        pagoda_nozzle = cylinder(d1=self.tube_diameter+self.inlet_thickness, \
                                    d2=self.tube_diameter-self.inlet_thickness, \
                                    h=self.tube_diameter, _fn=0, center=True)
        pagoda_nozzle_neg = cylinder(r=self.tube_diameter/2-self.inlet_thickness, \
    ...
    def build(self):
        return self.object
def _render_stl(scad_file, stl_file):
...
def spray_rig(
    ...
    for enum in ["middle", "endcap"]:
        Spray_Rig = SprayRig()\
                            ...
                            .fastener_diam(fastener_diameter)\
                            .fastener_gap(fastener_gap)\
                            .fastener_thick(fastener_thickness)\
                            .inter_span(interconnect_span)\
                            .center(True)\
                             .circle_arc_shell()\
                            .add_lid_fasteners()\
                            .add_interconnect()\
        ...
        renders.append((filename + ".scad", filename + ".stl"))
    header = "$fa = 1; $fs = 0.5;\n"
    for scad, _ in renders:
        with open(scad, "r+") as f:
            content = f.read()
            f.seek(0)
            f.write(header + content)
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
        futures = [pool.submit(_render_stl, scad, stl) for scad, stl in renders]
